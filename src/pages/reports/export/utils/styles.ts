...
export function styleHeaderRowAt(ws: any, headers: (string | number)[], rowIndex: number) {
  for (let c = 0; c < headers.length; c++) {
...
export function rightAlignNumbers(ws: any, startRow: number, startCol: number, endCol: number, useAmountFmt = false) {
  for (let r = startRow; ; r++) {
    ...
    for (let c = startCol; c <= endCol; c++) {
      ...
      if (ws[addr]) ws[addr].s = { ...(ws[addr].s || {}), alignment: { horizontal: 'right' }, numFmt: useAmountFmt ? amountFmt : numberFmt };
    }
  }
}
