                    ...
                    <CardContent className="space-y-3">
                        ...
                        {customer.images && customer.images.length > 0 && (
                            <div className="mt-4 pt-4 border-t border-primary/10">
                                <span className="text-sm text-muted-foreground block mb-2">Pictures</span>
                                <div className="flex gap-2 overflow-x-auto pb-2">
                                    {customer.images.map((url, i) => (
                                        <ImagePreview
                                            key={i}
                                            src={url}
                                            alt={`Customer picture ${i + 1}`}
                                            className="h-16 w-16 object-cover rounded-md border shadow-sm hover:opacity-80 transition-opacity shrink-0"
                                        />
                                    ))}
                                </div>
                            </div>
