...
export function TrialStatus({ trialEndDate, hasPurchased, className }: TrialStatusProps) {
  ...
  // If trial has expired (negative days), we might want to show "Expired" or nothing.
  // For now, let's assume we show "0 Days" if it's just expired, or negative.
  // But usually trial enforcement happens elsewhere.
  // Let's clamp to 0 minimum for display if we want to show "Trial Ended".
  // The user requirement is "Trial (15 Days)".
  
  let colorClass = 'text-muted-foreground'; // Default (> 7 days)
